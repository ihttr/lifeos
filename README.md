...
6. بعد أول نشر ناجح، أنشئ حسابك بتشغيل البذرة **من جهازك** مرة واحدة:

   ```bash
   DATABASE_URL="<رابط Neon المجمّع>" npm run db:seed
   ```

   البذرة لا تعمل على Vercel، ولا يجوز وضعها في أمر البناء لأنها تعيد زرع
   البيانات التجريبية عند كل نشر فتمحو تعديلاتك عليها.

7. `SEED_USER_*` لا يقرأها التطبيق المنشور — تُستخدم في البذرة محلياً فقط،
   فيمكنك حذفها من متغيرات Vercel بعد إنشاء الحساب.
